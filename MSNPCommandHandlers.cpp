...
*/
std::vector<std::string> MSNPCVR::executeCommand(std::string message, MSNClient& client, int dialectVersion) const {
	std::vector<std::string> responses;
	auto matches = StringUtils::splitWords(message);
	auto version = matches[7];
	auto clientMail = matches[9];
	auto commandOrder = matches[1];
	//TODO set client mail
...
*/
std::vector<std::string> MSNPUSR::executeCommand(std::string message, MSNClient& client, int dialectVersion) const {
	//SSO login
	std::vector<std::string> responses;
